...
- Low dependency depth (2 levels: ServiceDefaults �� Apps �� AppHost �� Tests)
...
**Package Updates:**
- ?? Aspire packages: 9.0.0 �� 13.1.0 (major version jump, well-documented upgrade path)
- ?? Entity Framework Core: 8.0.12 �� 10.0.1
- ?? ASP.NET Core Authentication: 8.0.0 �� 10.0.1
- ?? OpenTelemetry instrumentation: 1.9.0 �� 1.14.0
...
1. Update all 6 project files (TargetFramework: net9.0 �� net10.0)
...
```
Layer 0 (Foundation):
  ���� Ligot.ServiceDefaults (no dependencies)

Layer 1 (Application Services):
  ���� Ligot.ApiService �� ServiceDefaults
  ���� Ligot.BackEnd �� ServiceDefaults
  ���� Ligot.FrontEnd �� ServiceDefaults

Layer 2 (Orchestration):
  ���� Ligot.AppHost �� ApiService, BackEnd, FrontEnd
...
- **No circular dependencies** ?
- **Maximum depth:** 3 levels (Tests �� AppHost �� Services �� ServiceDefaults)
...
- Uses Aspire.Hosting.AppHost package (9.0.0 �� 13.1.0 major version jump)
...
**Target State:**
- Target Framework: net10.0
- Entity Framework Core packages updated: 8.0.12 �� 10.0.1 (2 major versions)
- Authentication package updated: 8.0.0 �� 10.0.1
...
- **EF Core 8 �� 10** is a 2-major-version jump
- **Potential Issues:**
...
**Target State:**
- Target Framework: net10.0
- Aspire.Hosting.AppHost: 9.0.0 �� 13.1.0 (major version jump)
...
**?? IMPORTANT:** Aspire 9.0 �� 13.1 is a **major version upgrade** (4 minor versions). Expect:
...
**Target State:**
- Target Framework: net10.0
- Aspire.Hosting.Testing: 9.0.0 �� 13.1.0
...
#### ?? Aspire.Hosting.AppHost 9.0 �� 13.1 (Major Version Jump)
...
#### ?? EF Core 8.0 �� 10.0 (2 Major Versions Jump)
...
3. **If Migration Errors Occur:**
   ...
   - Review EF Core 9 �� 10 breaking changes: [EF Core Breaking Changes](https://learn.microsoft.com/en-us/ef/core/what-is-new/ef-core-10.0/breaking-changes)
...
**Objective:** Verify EF Core 8 �� 10 upgrade doesn't break database operations
...
2. ? **Project-Level Validation** - Validate in dependency order (ServiceDefaults �� Services �� AppHost �� Tests)
...
**Stop Conditions:**
- If compilation fails �� Fix errors before proceeding to runtime tests
- If Aspire fails to launch �� Fix orchestration before testing services
- If automated tests fail �� Investigate failures before considering upgrade complete
...
| **Project-Level** | 6 projects �~ ~3 checks | Integration testing |
| **Integration** | Aspire orchestration + 4 services �~ ~5 checks | Automated tests |
...
- Aspire packages: 9.0.0 �� 13.1.0 (major version jump)
- Entity Framework Core: 8.0.12 �� 10.0.1
...
- TimeSpan.FromSeconds(Int64) �� TimeSpan.FromSeconds(double) in WebTests.cs
...
## Breaking Changes
- Fixed: TimeSpan.FromSeconds(Int64) �� TimeSpan.FromSeconds(double) in WebTests.cs
